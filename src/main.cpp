...
private:
    ...
    void createSwapChain() {
        ...
        QueueFamilyIndices indices = findQueueFamilies(physicalDevice);
        uint32_t queueFamilyIndicesArr[2] = { indices.graphicsFamily.value(), indices.presentFamily.value() };
    ...
    void createTextureImage() {
        // Load PNG file
        FILE* png_file = fopen("assets/minecraft/textures/texture.png", "rb");
        if (!png_file) {
        ...
        int texWidth = ihdr.width;
        int texHeight = ihdr.height;

        mipLevels = static_cast<uint32_t>(std::floor(std::log2(std::max(texWidth, texHeight)))) + 1;

        // Calculate decoded image size for RGBA
        size_t out_size;
        ...
        VkDeviceSize imageSize = out_size;
        std::vector<unsigned char> pixels(out_size);

        // Decode to RGBA8 without any transformations
        ret = spng_decode_image(ctx, pixels.data(), out_size, SPNG_FMT_RGBA8, SPNG_DECODE_TRNS);
        if (ret) {
            spng_ctx_free(ctx);
    ...
    void createGraphicsPipeline() {
        auto meshShaderCode = readFile("assets/minecraft/shaders/triangle.msh.spv");
        auto fragShaderCode = readFile("assets/minecraft/shaders/triangle.fsh.spv");
