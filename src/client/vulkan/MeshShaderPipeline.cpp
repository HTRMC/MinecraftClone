...
void MeshShaderPipeline::initWithDynamicRendering(VkFormat colorFormat, VkFormat depthFormat) {
    if (initialized) return;
    
    createPipelineWithDynamicRendering(colorFormat, depthFormat);
    
    initialized = true;
    Logger::info("MeshShaderPipeline", "Initialized mesh shader pipeline with dynamic rendering");
}
...
void MeshShaderPipeline::createPipelineWithDynamicRendering(VkFormat colorFormat, VkFormat depthFormat) {
    VkPipelineShaderStageCreateInfo meshShaderStageInfo{};
    meshShaderStageInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    meshShaderStageInfo.stage = VK_SHADER_STAGE_MESH_BIT_EXT;
    meshShaderStageInfo.module = shaderManager->getMeshShaderModule();
    ...
    if (vkCreateGraphicsPipelines(vulkanContext->getDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create mesh shader graphics pipeline with dynamic rendering!");
    }
}
